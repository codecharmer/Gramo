 ...
 *
 * Registers the `wp gramo` command family and wires the Settings-screen
 * action (`gramo_run_content_install`) to the installer. Nothing is seeded
 * automatically on a normal request: activation only raises the
 * `gramo_needs_content_install` flag, and this class surfaces an admin notice
...
namespace Gramo\Core\Cli;

use Gramo\Core\Contracts\Bootable;
use Gramo\Core\Setup\Installer;
use Gramo\Core\Setup\MediaImporter;
...
final class Commands implements Bootable {
	...
	private const ADMIN_ACTION = 'gramo_install_content';

	public function boot(): void {
		// Settings screen (and the notice button) fire this action.
		add_action( 'gramo_run_content_install', array( $this, 'handle_run_content_install' ) );
		...
		if ( defined( 'WP_CLI' ) && \WP_CLI ) {
			\WP_CLI::add_command( 'gramo install', array( $this, 'install' ) );
			\WP_CLI::add_command( 'gramo seed-products', array( $this, 'seed_products' ) );
			\WP_CLI::add_command( 'gramo install-pages', array( $this, 'install_pages' ) );
			\WP_CLI::add_command( 'gramo import-media', array( $this, 'import_media' ) );
			\WP_CLI::add_command( 'gramo reset', array( $this, 'reset' ) );
		}
	}
	...
	/**
	 * Hooked to `gramo_run_content_install`. Runs the full install.
	 *
	...
	public function render_install_notice(): void {
		if ( ! get_option( 'gramo_needs_content_install' ) ) {
			return;
		}
		...
		echo esc_html__( 'Gramo está lista para instalar el contenido inicial (productos, páginas y menús).', 'gramo-core' );
		echo ' <a class="button button-primary" href="' . esc_url( $url ) . '">';
		echo esc_html__( 'Instalar contenido de Gramo', 'gramo-core' );
		echo '</a></p></div>';
	}
	...
	public function handle_admin_post(): void {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'No tienes permiso para realizar esta acción.', 'gramo-core' ) );
		}
	...
	/**
	 * Install all Gramo seed content: options, categories, products, pages,
	 ...
	 * ## EXAMPLES
	 *
	 *     wp gramo seed-products
	 *     wp gramo seed-products --force
	 *
	 * @when after_wp_load
	 *
	 * @param array<int,string>    $args
	 ...
	 * ## EXAMPLES
	 *
	 *     wp gramo install-pages
	 *     wp gramo install-pages --force
	 *
	 * @when after_wp_load
	 *
	 * @param array<int,string>    $args
	 ...
	 *     wp gramo import-media
	 *
	 *     # Force a full re-import
	 *     wp gramo import-media --force
	 *
	 * @when after_wp_load
	 *
	 * @param array<int,string>    $args
	...
	/**
	 * Remove all Gramo-seeded content (products, pages, navigation, media,
	...
	public function reset( array $args, array $assoc_args ): void {
		...
		\WP_CLI::success( 'Contenido de Gramo eliminado.' );
	}
