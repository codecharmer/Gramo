...
final class MetaTags implements Bootable {
	...
	public function render(): void {
		...
		$post_object = $this->queried_object();
		$type        = $this->og_type();
		$title       = $this->document_title();
		$desc        = $this->description( $post_object, $business );
		$url         = $this->canonical_url();
		$locale      = 'es_MX';
		$site        = (string) ( $business['name'] ?? get_bloginfo( 'name' ) );
		$image       = $this->image( $post_object, $seo );
		...
		if ( 'product' === $type ) {
			$price = $this->product_price( $post_object );
			if ( null !== $price ) {
	...
	private function seo_plugin_active(): bool {
		...
		/**
		 * Allow a site to force-disable Gramo meta output (e.g. another plugin).
		 *
		 * @param bool $active Whether a competing SEO solution is active.
		 */
		return (bool) apply_filters( 'gramo_seo_meta_suppressed', false );
	}
	 ...
	 */
	private function description( ?\WP_Post $post_object, array $business ): string {
		$raw = '';

		if ( $post_object instanceof \WP_Post ) {
			// WooCommerce product short description takes priority.
			if ( 'product' === $post_object->post_type && function_exists( 'wc_get_product' ) ) {
			...
			if ( '' === trim( wp_strip_all_tags( $raw ) ) ) {
				$raw = has_excerpt( $post_object ) ? (string) get_the_excerpt( $post_object ) : (string) $post_object->post_content;
			}
	 ...
	 */
	private function image( ?\WP_Post $post_object, array $seo ): ?array {
		$attachment_id = 0;

		if ( $post_object instanceof \WP_Post && has_post_thumbnail( $post_object ) ) {
			$attachment_id = (int) get_post_thumbnail_id( $post_object );
		}
		...
		if ( '' === trim( $alt ) && $post_object instanceof \WP_Post ) {
			$alt = (string) get_the_title( $post_object );
		}
	...
	private function robots(): string {
		 ...
		 */
		$directives = (array) apply_filters( 'gramo_seo_robots', $directives );
	 ...
	 */
	private function product_price( ?\WP_Post $post_object ): ?array {
		if ( ! $post_object instanceof \WP_Post || ! function_exists( 'wc_get_product' ) ) {
			return null;
		}
		$product = wc_get_product( $post_object );
		if ( ! $product instanceof \WC_Product ) {
