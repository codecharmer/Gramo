 ...
 * business (CafeOrCoffeeShop/LocalBusiness), the website, the current page, and — where
...
final class SchemaGraph implements Bootable {
	...
	public function render(): void {
		...
		echo "\n<!-- Gramo SEO: schema -->\n";
		echo '<script type="application/ld+json">' . $json . "</script>\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wp_json_encode() with JSON_HEX_TAG|JSON_HEX_AMP cannot break out of the script element.
	}
	...
	/**
	 * The CafeOrCoffeeShop (LocalBusiness/Organization) node — the business identity.
	 *
	...
	private function breadcrumb_items(): array {
		...
		if ( is_search() ) {
			$items[] = array(
				...
				'name' => sprintf( __( 'Resultados para "%s"', 'gramo-core' ), get_search_query() ),
				'url'  => '',
			);
			return $items;
		}
