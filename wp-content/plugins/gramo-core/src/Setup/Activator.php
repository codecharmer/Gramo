...
final class Activator {
	...
	public const SMS_TABLE = 'gramo_sms_log';

	/** Capability that gates the Gramo operations dashboard. */
	public const CAP = 'manage_gramo';
	...
	public static function activate(): void {
		...
		if ( false === get_option( 'gramo_content_installed' ) ) {
			add_option( 'gramo_needs_content_install', 1 );
		}

		if ( ! wp_next_scheduled( 'gramo_daily_maintenance' ) ) {
			wp_schedule_event( time() + HOUR_IN_SECONDS, 'daily', 'gramo_daily_maintenance' );
		}
	...
	public static function deactivate(): void {
		wp_clear_scheduled_hook( 'gramo_daily_maintenance' );
		flush_rewrite_rules();
	}
