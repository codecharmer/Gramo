 ...
 *
 * Orchestrates the one-command provisioning of Gramo's storefront: options,
 ...
 * screen (via the `gramo_run_content_install` action), or from an admin
...
final class Installer {
	...
	public const MARKER_META = '_gramo_seeded';

	/** Product meta flagging an estimated price (mirrors brand-brief §5). */
	public const PRICE_ESTIMATE_META = '_gramo_price_estimate';

	/** Meta storing the authored SEO description. */
	public const META_DESC = '_gramo_meta_description';

	/** Meta storing the short SEO summary. */
	public const SEO_SHORT = '_gramo_seo_short';
	 ...
	 * everything else.
	 *
	 * The source is the ink mark on transparency (`marca-gramo.png`); the
	...
	public static function install_branding(): array {
		$logo_id = MediaImporter::ensure( 'marca-gramo', 'Gramo' );

		if ( $logo_id > 0 ) {
			set_theme_mod( 'custom_logo', $logo_id );
		}

		// Seed the favicon / site icon too, so it survives a fresh install like
		// the logo does. Prefer a dedicated square icon (data/media/source/
		// favicon.png); fall back to the logo mark if none is supplied.
		$icon_id = MediaImporter::ensure( 'favicon', 'Gramo' );
		if ( $icon_id <= 0 ) {
			$icon_id = $logo_id;
		}
		if ( $icon_id > 0 ) {
	...
	public static function install_pages( bool $force ): array {
		...
		foreach ( $pages as $def ) {
			...
			if ( $existing_id > 0 && ! $force ) {
				...
				$postarr = array(
					'ID'           => $existing_id > 0 ? $existing_id : 0,
					'post_type'    => 'page',
					'post_status'  => 'publish',
	...
	public static function install_navigation(): array {
		...
		if ( ! empty( $nav['primary'] ) ) {
			...
			update_option( 'gramo_primary_nav_id', $out['primary_id'] );
		}
		if ( ! empty( $nav['footer'] ) ) {
			$out['footer_id'] = self::upsert_navigation_post( 'pie', 'Pie de página', (array) $nav['footer'] );
			update_option( 'gramo_footer_nav_id', $out['footer_id'] );
		}
	...
	---------------------------------------------------------------------- */
	/*
	Reset (used by `wp gramo reset --yes`)                               */
	/* ---------------------------------------------------------------------- */
	...
	public static function reset(): array {
		...
		delete_option( 'gramo_content_installed' );
		delete_option( 'gramo_primary_nav_id' );
		delete_option( 'gramo_footer_nav_id' );
	 ...
	 * `wp gramo import-media` command to (re-)import the full media set.
	 *
	...
	private static function data( string $name ): array {
		...
		$file = trailingslashit( GRAMO_CORE_DIR ) . 'data/' . $name . '.php';
		if ( ! is_readable( $file ) ) {
			return array();
		}
		$data = require $file;
	...
	private static function log( array $report ): void {
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( '[gramo-core] Instalación de contenido: ' . wp_json_encode( $report ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Guarded by WP_DEBUG; local diagnostics only.
		}
	}
}
