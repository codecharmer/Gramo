 ...
 * Gramo photograph at `data/media/source/{image_key}.{jpg,png,webp}` we
...
final class MediaImporter {
	...
	public const CACHE_OPTION = 'gramo_media_map';

	/** Marker meta stamped on every attachment we create (used by reset). */
	public const MARKER_META = '_gramo_seeded_media';
	...
	public static function source_dir(): string {
		$dir = trailingslashit( GRAMO_CORE_DIR ) . 'data/media/source';
		if ( ! is_dir( $dir ) ) {
			wp_mkdir_p( $dir );
		}
	...
	public static function generate_responsive( int $attachment_id ): array {
		 ...
		 */
		do_action( 'gramo_media_responsive_generated', $attachment_id, $generated );
	...
	private static function sideload_file( string $path, string $filename, string $alt ): int {
		...
		$detected = (string) wp_check_filetype( $upload['file'] )['type'];
		$mime     = 'svg' === $ext ? 'image/svg+xml' : ( '' !== $detected ? $detected : 'application/octet-stream' );
