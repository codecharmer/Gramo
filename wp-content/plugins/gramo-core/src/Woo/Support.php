...
/**
 * Configures WooCommerce for a specialty coffee shop in MXN (pickup and local delivery):
 *
...
final class Support implements Bootable {
	...
	public function register_order_statuses(): void {
		register_post_status(
			...
			array(
				'label'                     => _x( 'Preparando', 'Order status', 'gramo-core' ),
				'public'                    => false,
				'internal'                  => false,
				...
				'label_count'               => _n_noop( 'Preparando <span class="count">(%s)</span>', 'Preparando <span class="count">(%s)</span>', 'gramo-core' ),
			)
		);
		...
		register_post_status(
			self::STATUS_READY,
			array(
				'label'                     => _x( 'Listo para recoger', 'Order status', 'gramo-core' ),
				'public'                    => false,
				'internal'                  => false,
				'exclude_from_search'       => false,
				'show_in_admin_all_list'    => true,
				'show_in_admin_status_list' => true,
				/* translators: %s: order count. */
				'label_count'               => _n_noop( 'Listo para recoger <span class="count">(%s)</span>', 'Listo para recoger <span class="count">(%s)</span>', 'gramo-core' ),
			)
		);
	}
	...
	public function add_order_statuses( array $statuses ): array {
		...
		foreach ( $statuses as $key => $label ) {
			...
			if ( 'wc-processing' === $key ) {
				$ordered[ self::STATUS_PREPARING ] = _x( 'Preparando', 'Order status', 'gramo-core' );
				$ordered[ self::STATUS_READY ]     = _x( 'Listo para recoger', 'Order status', 'gramo-core' );
			}
		}
	...
	public function pickup_only_notice(): void {
		...
		wc_print_notice(
			esc_html__( 'Todos los pedidos son para recoger en tienda (Calle Ejemplo 000). No hay envíos.', 'gramo-core' ),
			'notice'
		);
	}
	...
	public function localize_out_of_stock( string $text, $product ): string {
		if ( $product instanceof \WC_Product && ! $product->is_in_stock() ) {
			return esc_html__( 'Agotado', 'gramo-core' );
		}
		return $text;
	}
}
