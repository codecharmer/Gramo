...
final class Inventory implements Bootable {
	...
	public function loop_add_to_cart_link( string $html, $product ): string {
		if ( $product instanceof \WC_Product && ! $product->is_in_stock() ) {
			return sprintf(
				'<span class="button gramo-agotado disabled" aria-disabled="true">%s</span>',
				esc_html__( 'Agotado', 'gramo-core' )
			);
		}
		return $html;
	}
	...
	public function single_sold_out_button(): void {
		...
		printf(
			'<p class="gramo-agotado-single"><button type="button" class="button disabled" disabled aria-disabled="true">%s</button></p>',
			esc_html__( 'Agotado', 'gramo-core' )
		);
	}
	...
	/**
	 * Re-broadcast WooCommerce's low-stock event on a Gramo-namespaced hook so
	...
	public function on_low_stock( $product ): void {
		 ...
		 */
		do_action( 'gramo_low_stock_ping', $product );
	}
}
