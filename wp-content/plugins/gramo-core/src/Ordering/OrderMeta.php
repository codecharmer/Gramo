...
final class OrderMeta implements Bootable {
	...
	public function render_admin( $order ): void {
		...
		echo '<div class="gramo-pickup-admin"><p><strong>';
		echo esc_html__( 'Recolección:', 'gramo-core' );
		echo '</strong><br>';
		echo esc_html( $label );
		echo '</p></div>';
	}
	...
	public function render_frontend( $order ): void {
		...
		echo '<section class="gramo-pickup-details woocommerce-order-details">';
		echo '<h2 class="woocommerce-order-details__title">' . esc_html__( 'Recolección en tienda', 'gramo-core' ) . '</h2>';
		echo '<p>' . esc_html__( 'Fecha y horario:', 'gramo-core' ) . ' <strong>' . esc_html( $label ) . '</strong></p>';
		echo '</section>';
	}
	...
	public function render_email( $order, $sent_to_admin = false, $plain_text = false, $email = null ): void {
		...
		if ( $plain_text ) {
			echo "\n" . esc_html__( 'Recolección en tienda', 'gramo-core' ) . "\n";
			echo esc_html__( 'Fecha y horario:', 'gramo-core' ) . ' ' . esc_html( $label ) . "\n";
			return;
		}

		echo '<div style="margin-bottom:24px;">';
		echo '<h2 style="color:#2E2016;font-family:serif;">' . esc_html__( 'Recolección en tienda', 'gramo-core' ) . '</h2>';
		echo '<p style="font-size:15px;">' . esc_html__( 'Fecha y horario:', 'gramo-core' ) . ' <strong>' . esc_html( $label ) . '</strong></p>';
		echo '</div>';
	}
}
