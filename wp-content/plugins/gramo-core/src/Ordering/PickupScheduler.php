...
final class PickupScheduler implements Bootable {
	...
	public const META_DATE  = '_gramo_pickup_date';
	public const META_SLOT  = '_gramo_pickup_slot';
	public const META_LABEL = '_gramo_pickup_label';

	/** Classic checkout POST field names. */
	private const FIELD_DATE = 'gramo_pickup_date';
	private const FIELD_SLOT = 'gramo_pickup_slot';

	/** Block checkout additional-field ids (namespace gramo/pickup). */
	private const BLOCK_DATE = 'gramo/pickup-date';
	private const BLOCK_SLOT = 'gramo/pickup-slot';
	...
	public function render_classic_fields(): void {
		...
		echo '<div id="gramo-pickup" class="gramo-pickup-fields">';
		echo '<h3>' . esc_html__( 'Recolección del pedido', 'gramo-core' ) . '</h3>';
		echo '<p class="gramo-pickup-help">' . esc_html__( 'Elige el día y el horario para recoger tu pedido en tienda.', 'gramo-core' ) . '</p>';

		if ( empty( $dates ) ) {
			echo '<p class="gramo-pickup-empty">' . esc_html__( 'Por el momento no hay fechas de recolección disponibles. Contáctanos para coordinar tu pedido.', 'gramo-core' ) . '</p>';
			echo '</div>';
			return;
		}
		...
		echo '<label for="' . esc_attr( self::FIELD_DATE ) . '">' . esc_html__( 'Fecha de recolección', 'gramo-core' ) . ' <abbr class="required" title="' . esc_attr__( 'obligatorio', 'gramo-core' ) . '">*</abbr></label>';
		echo '<select name="' . esc_attr( self::FIELD_DATE ) . '" id="' . esc_attr( self::FIELD_DATE ) . '" class="select" required>';
		echo '<option value="">' . esc_html__( 'Selecciona una fecha…', 'gramo-core' ) . '</option>';
		foreach ( $dates as $d ) {
		...
		echo '<label for="' . esc_attr( self::FIELD_SLOT ) . '">' . esc_html__( 'Horario', 'gramo-core' ) . ' <abbr class="required" title="' . esc_attr__( 'obligatorio', 'gramo-core' ) . '">*</abbr></label>';
		echo '<select name="' . esc_attr( self::FIELD_SLOT ) . '" id="' . esc_attr( self::FIELD_SLOT ) . '" class="select" required>';
		echo '<option value="">' . esc_html__( 'Selecciona un horario…', 'gramo-core' ) . '</option>';
	...
	private function print_classic_script( array $dates ): void {
		...
		$empty   = esc_js( __( 'Selecciona un horario…', 'gramo-core' ) );
		$date_id = esc_js( self::FIELD_DATE );
		$slot_id = esc_js( self::FIELD_SLOT );

		// phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- $json is wp_json_encode() output; the id/label vars are esc_js()-escaped above.
	 ...
	 * `gramo/pickup` namespace, when the Blocks API is available.
	 */
	public function register_block_fields(): void {
		...
		try {
			woocommerce_register_additional_checkout_field(
				array(
					'id'       => self::BLOCK_DATE,
					'label'    => __( 'Fecha de recolección', 'gramo-core' ),
					'location' => 'order',
					'type'     => 'select',
					'required' => true,
			...
			woocommerce_register_additional_checkout_field(
				array(
					'id'       => self::BLOCK_SLOT,
					'label'    => __( 'Horario de recolección', 'gramo-core' ),
					'location' => 'order',
					'type'     => 'select',
					'required' => true,
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( '[gramo-core] Block pickup fields not registered: ' . $e->getMessage() ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Guarded by WP_DEBUG; local diagnostics only.
			}
		}
	}
	...
	public function validate_block_fields( $errors, $fields ): void {
		...
		$error = self::validation_error( $date, $slot );
		if ( null !== $error ) {
			$errors->add( 'gramo_pickup_invalid', $error );
		}
	}
	...
	private static function validation_error( string $date, string $slot ): ?string {
		if ( '' === $date || '' === $slot ) {
			return esc_html__( 'Selecciona una fecha y un horario de recolección.', 'gramo-core' );
		}
		if ( ! self::is_open_date( $date ) ) {
			return esc_html__( 'El día seleccionado no está disponible para recolección. Elige otra fecha.', 'gramo-core' );
		}
		if ( ! in_array( $slot, self::all_slots(), true ) ) {
			return esc_html__( 'El horario seleccionado no es válido. Elige otro.', 'gramo-core' );
		}

		$available = array_column( self::available_slots( $date ), 'slot' );
		if ( ! in_array( $slot, $available, true ) ) {
			return esc_html__( 'Ese horario ya no está disponible (cupo lleno o fuera del tiempo de anticipación). Elige otro.', 'gramo-core' );
		}
