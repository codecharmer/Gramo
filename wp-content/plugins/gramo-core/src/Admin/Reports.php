...
final class Reports implements Bootable {
	...
	private const SLUG = 'gramo-reportes';
	...
	public function render(): void {
		...
		$ranges = array(
			'today' => __( 'Hoy', 'gramo-core' ),
			'7d'    => __( 'Últimos 7 días', 'gramo-core' ),
			'30d'   => __( 'Últimos 30 días', 'gramo-core' ),
		);
		...
		?>
		<div class="wrap gramo-wrap gramo-reports">
			<h1 class="gramo-title">
				<span class="dashicons dashicons-chart-bar" aria-hidden="true"></span>
				<?php esc_html_e( 'Reportes', 'gramo-core' ); ?>
			</h1>

			<div class="gramo-tabs" role="tablist">
				<?php foreach ( $ranges as $key => $label ) : ?>
					<a class="gramo-range-tab <?php echo $range === $key ? 'is-active' : ''; ?>" href="<?php echo esc_url( add_query_arg( 'range', $key, $base_url ) ); ?>"><?php echo esc_html( $label ); ?></a>
				<?php endforeach; ?>
			</div>

			<div class="gramo-cards">
				<?php
				$this->stat_card( $this->money( $report['revenue'] ), __( 'Ingresos', 'gramo-core' ), 'dashicons-money-alt' );
				$this->stat_card( (string) $report['orders'], __( 'Pedidos', 'gramo-core' ), 'dashicons-cart' );
				$this->stat_card( $this->money( $report['avg'] ), __( 'Ticket promedio', 'gramo-core' ), 'dashicons-tag' );
				?>
			</div>

			<div class="gramo-grid">
				<section class="gramo-panel gramo-panel--wide">
					<h2 class="gramo-panel__title"><?php esc_html_e( 'Ingresos por día', 'gramo-core' ); ?></h2>
					<?php $this->render_bar_chart( $report['series'] ); ?>
				</section>

				<section class="gramo-panel">
					<h2 class="gramo-panel__title"><?php esc_html_e( 'Productos más vendidos', 'gramo-core' ); ?></h2>
					<?php $this->render_top_products( $report['top_products'] ); ?>
				</section>
			</div>

			<section class="gramo-panel gramo-panel--wide">
				<h2 class="gramo-panel__title"><?php esc_html_e( 'Historial SMS', 'gramo-core' ); ?></h2>
	...
	private function stat_card( string $value, string $label, string $icon ): void {
		printf(
			'<div class="gramo-card"><span class="dashicons %1$s" aria-hidden="true"></span><span class="gramo-card__value">%2$s</span><span class="gramo-card__label">%3$s</span></div>',
			esc_attr( $icon ),
			wp_kses_post( $value ),
			esc_html( $label )
		);
	...
	private function render_bar_chart( array $series ): void {
		if ( empty( $series ) || array_sum( $series ) <= 0 ) {
			echo '<p class="gramo-empty">' . esc_html__( 'Sin datos para graficar en este rango.', 'gramo-core' ) . '</p>';
			return;
		}
		...
		echo '<div class="gramo-chart"><svg viewBox="0 0 ' . esc_attr( (string) $width ) . ' ' . esc_attr( (string) $height ) . '" role="img" preserveAspectRatio="xMidYMid meet" aria-label="' . esc_attr__( 'Ingresos por día', 'gramo-core' ) . '">';
		...
		foreach ( $series as $date => $value ) {
			...
			printf(
				'<rect class="gramo-bar" x="%1$s" y="%2$s" width="%3$s" height="%4$s" rx="3"><title>%5$s: %6$s</title></rect>',
			...
			printf(
				'<text class="gramo-bar-label" x="%1$s" y="%2$s" text-anchor="middle">%3$s</text>',
	...
	private function render_top_products( array $top ): void {
		if ( empty( $top ) ) {
			echo '<p class="gramo-empty">' . esc_html__( 'Sin ventas en este rango.', 'gramo-core' ) . '</p>';
			return;
		}
		echo '<table class="widefat striped gramo-table"><thead><tr>';
		echo '<th>' . esc_html__( 'Producto', 'gramo-core' ) . '</th>';
		echo '<th>' . esc_html__( 'Unidades', 'gramo-core' ) . '</th>';
		echo '</tr></thead><tbody>';
	...
	private function render_sms_history(): void {
		if ( ! class_exists( Logger::class ) || ! method_exists( Logger::class, 'recent' ) ) {
			echo '<p class="gramo-empty">' . esc_html__( 'El módulo de SMS aún no está disponible.', 'gramo-core' ) . '</p>';
			return;
		}
		...
		echo '<th>' . esc_html__( 'Pedido', 'gramo-core' ) . '</th>';
