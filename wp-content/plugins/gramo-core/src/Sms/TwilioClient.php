 ...
 * which resolves GRAMO_TWILIO_* constants over stored values. Secrets are
...
final class TwilioClient {
	...
	public function send( string $to, string $body ): array {
		...
		if ( empty( $sms['enabled'] ) ) {
			return self::failure( __( 'La mensajería SMS está desactivada.', 'gramo-core' ) );
		}
		...
		$response = wp_remote_post(
			...
			array(
				'timeout' => 15,
				'headers' => array(
					'Authorization' => 'Basic ' . base64_encode( $sid . ':' . $token ), // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode -- HTTP Basic auth encoding, not obfuscation.
		...
		$message = isset( $data['message'] ) && '' !== (string) $data['message']
			...
			: sprintf( __( 'Error de Twilio (HTTP %d).', 'gramo-core' ), $code );
	...
	public static function validate_signature( string $auth_token, string $url, array $params, string $signature ): bool {
		...
		$computed = base64_encode( hash_hmac( 'sha1', $data, $auth_token, true ) ); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.obfuscation_base64_encode -- Twilio's documented signature scheme.
	 ...
	 *
	 * WooCommerce billing phones arrive in any format and often without a
	 * country code; passing them raw makes the provider guess the country
	 * (it defaults to US +1), which silently misroutes local numbers. Keep a
	 * number that already carries a `+`; otherwise strip formatting and prepend
	 * the store's country dialing code (from Options::business()['country'],
	 * defaulting to Mexico). Adjust STARTER default per brand/market.
	 */
	private static function to_e164( string $number ): string {
		$number = trim( $number );
		if ( '' === $number ) {
			return '';
		}
		...
		// Some locales include a national trunk prefix "00" for international.
		...
		$country = strtoupper( (string) ( Options::business()['country'] ?? 'MX' ) );
		$code    = $dial[ $country ] ?? '52';

		// If the number already begins with the country code, don't double it.
		if ( 0 === strpos( $digits, $code ) && strlen( $digits ) > 10 ) {
			return '+' . $digits;
		}
		return '+' . $code . $digits;
	}
