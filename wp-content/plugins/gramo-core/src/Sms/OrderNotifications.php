 ...
 * via `gramo_sms_message` so it can be edited without touching code.
 *
 * @package Gramo\Core
 */
...
use Gramo\Core\Ordering\OrderMeta;
...
final class OrderNotifications implements Bootable {
	...
	private const STAFF_FLAG = '_gramo_staff_notified';
	...
	private function staff_message( \WC_Order $order ): string {
		...
		if ( '' === $customer ) {
			$customer = __( 'Cliente', 'gramo-core' );
		}

		$default = sprintf(
			/* translators: 1: order number, 2: customer name, 3: item summary, 4: pickup label. */
			__( "NUEVO PEDIDO\n#%1\$s\n%2\$s\n%3\$s\nRecoge: %4\$s\n\nResponde:\n1=Preparando 2=Listo 3=Entregado 4=Cancelado", 'gramo-core' ),
		...
		switch ( $status ) {
			case 'processing':
				$default = sprintf(
					...
					__( '¡Gracias! Recibimos tu pedido #%s. Te avisaremos cuando esté listo. 🥖', 'gramo-core' ),
					$number
				);
				break;
			case 'preparing':
				$default = sprintf(
					/* translators: %s: order number. */
					__( 'Manos a la masa: estamos preparando tu pedido #%s.', 'gramo-core' ),
					$number
				);
				break;
			case 'ready':
				$default = sprintf(
					...
					__( '¡Tu pedido #%1$s está listo para recoger! Te esperamos en %2$s.', 'gramo-core' ),
					$number,
					$address
				);
				break;
			case 'completed':
				$default = sprintf(
					/* translators: %s: order number. */
					__( '¡Gracias por tu compra! Entregamos tu pedido #%s. ¡Buen provecho! 🌾', 'gramo-core' ),
					$number
				);
				break;
			case 'cancelled':
				$default = sprintf(
					/* translators: %s: order number. */
					__( 'Tu pedido #%s fue cancelado. Si tienes dudas, contáctanos.', 'gramo-core' ),
					$number
				);
				break;
			default:
	...
	private function pickup_label( \WC_Order $order ): string {
		...
		return __( 'por confirmar', 'gramo-core' );
	}
	...
	private function items_summary( \WC_Order $order ): string {
		...
		return '' !== $summary ? $summary : __( '(sin artículos)', 'gramo-core' );
	}
	...
	private function template( string $key, string $fallback, \WC_Order $order ): string {
		 ...
		 */
		return (string) apply_filters( 'gramo_sms_message', $fallback, $key, $order );
	}
}
