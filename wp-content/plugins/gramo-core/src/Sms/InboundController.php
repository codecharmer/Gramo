 ...
 *
 * Registers POST `gramo/v1/sms/inbound`. The route is publicly reachable but
...
final class InboundController implements Bootable {
	...
	private const NAMESPACE = 'gramo/v1';
	...
	public function handle( \WP_REST_Request $request ) {
		...
		if ( ! TwilioClient::validate_signature( $token, $this->request_url(), $params, $signature ) ) {
			return new \WP_Error(
				'gramo_sms_bad_signature',
				__( 'Firma de Twilio inválida.', 'gramo-core' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( ! $this->is_staff_number( $from, (array) ( $sms['staff_numbers'] ?? array() ) ) ) {
			...
			return $this->twiml( __( 'Número no autorizado.', 'gramo-core' ) );
		}
		...
		if ( null === $status ) {
			...
			return $this->twiml(
				__( 'No entendí el mensaje. Responde 1=Preparando 2=Listo 3=Entregado 4=Cancelado.', 'gramo-core' )
			);
		}
		...
		if ( ! $order instanceof \WC_Order ) {
			...
			return $this->twiml( __( 'No encontré un pedido activo para actualizar.', 'gramo-core' ) );
		}
		...
		$order->update_status(
			...
			sprintf(
				...
				__( 'Actualizado por SMS desde %s', 'gramo-core' ),
				$from
			)
		);
