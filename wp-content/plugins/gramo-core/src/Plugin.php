...
final class Plugin {
	...
	public function boot(): void {
		...
		load_plugin_textdomain( 'gramo-core', false, dirname( plugin_basename( GRAMO_CORE_FILE ) ) . '/languages' );

		foreach ( $this->service_classes() as $class ) {
			if ( ! class_exists( $class ) ) {
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					error_log( sprintf( '[gramo-core] Service not found (skipped): %s', $class ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- Guarded by WP_DEBUG; local diagnostics only.
				}
				continue;
			}
			$service = new $class();
		 ...
		 */
		do_action( 'gramo_core_booted', $this );
	}
