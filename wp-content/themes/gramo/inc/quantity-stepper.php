 ...
 */
function gramo_qty_module(): void {
	if ( is_admin() || ! function_exists( 'wp_enqueue_script_module' ) ) {
		return;
	}
	wp_enqueue_script_module(
		'gramo-qty',
		GRAMO_THEME_URI . '/assets/js/quantity.js',
		array( '@wordpress/interactivity' ),
		gramo_asset_version( 'assets/js/quantity.js' )
	);
}
add_action( 'wp_enqueue_scripts', 'gramo_qty_module' );
 ...
 * @return string
 */
function gramo_qty_stepper( string $content ): string {
	...
	$stepper = sprintf(
		'<div class="pf-qty" data-wp-interactive="gramo/qty">' .
		...
		'</div>',
		esc_attr__( 'Reducir cantidad', 'gramo' ),
		esc_attr__( 'Aumentar cantidad', 'gramo' )
	);
...
add_filter( 'render_block_woocommerce/product-button', 'gramo_qty_stepper' );
