<?php
/**
 * Title: Frase de firma
 * Slug: gramo/signature
 * Categories: gramo-page
 * Description: Banda a sangre completa en rosa de marca con una frase de firma en tipografía display. El color es la voz.
 * Keywords: firma, frase, lema, rosa, masa madre
 * Viewport Width: 1400
 */
?>
<!-- wp:group {"align":"full","backgroundColor":"rosa","textColor":"crust","className":"pf-signature","style":{"spacing":{"padding":{"top":"var:preset|spacing|xxl","bottom":"var:preset|spacing|xxl"}}},"layout":{"type":"constrained"}} -->
<div class="wp-block-group alignfull pf-signature has-crust-color has-rosa-background-color has-text-color has-background" style="padding-top:var(--wp--preset--spacing--xxl);padding-bottom:var(--wp--preset--spacing--xxl)"><!-- wp:paragraph {"className":"pf-eyebrow"} -->
<p class="pf-eyebrow">Gramo · pan de masa madre</p>
<!-- /wp:paragraph -->

<!-- wp:heading {"level":2,"fontSize":"xxl"} -->
<h2 class="wp-block-heading has-xxl-font-size">No hacemos pan rápido. Hacemos <em>buen</em> pan.</h2>
<!-- /wp:heading --></div>
<!-- /wp:group -->
